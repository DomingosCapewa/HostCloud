...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_plano'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE planos SET nome = ?, descricao = ?, preco = ?, armazenamento = ?, largura_banda = ?, sites = ?, contas_email = ?, banco_dados = ?, dominio_gratis = ? WHERE id = ?");
        $stmt->execute([$nome, $descricao, $preco, $armazenamento, $largura_banda, $sites, $contas_email, $banco_dados, $dominio_gratis, $id]);
